"""Main entry point for AgentWinter"""
import sys
...
def handle_file_change(file_path, event_type, context_ref, parsed_files_ref, symbol_index_ref):
    ...
    try:
        ...
        print(f"{COLORS['GREEN']}✅ Updated! ({len(new_symbol_index)} symbols){COLORS['RESET']}\n")
...
def main():
    ...
    print("="*70)
    print(f"{COLORS['CYAN']}💡 AGENTWINTER - Project Context Aware{COLORS['RESET']}")
    print("="*70)
    ...
    if watch_mode:
        try:
            ...
            print(f"{COLORS['YELLOW']}⚠️  File watching unavailable: {e}{COLORS['RESET']}")
