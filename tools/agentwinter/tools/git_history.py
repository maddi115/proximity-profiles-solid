...
def git_log(file_path, limit=10):
    ...
    try:
        result = subprocess.run(
            ['git', 'log', f'-{limit}', '--format=%H|%an|%ae|%ad|%s', '--date=short', '--', file_path],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def git_blame(file_path, start_line=None, end_line=None):
    ...
    try:
        ...
        return {
            "file": file_path,
            "lines": lines[:50] if not start_line else lines  # Limit to 50 lines if full file
        }
...
def git_recent_changes(since="1 week", path=None):
    ...
    """
    try:
        cmd = ['git', 'log', f'--since={since}', '--format=%H|%an|%ad|%s', '--date=short']
